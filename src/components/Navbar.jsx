...
import { AiOutlineUser } from 'react-icons/ai'; // 추가

export default function Navbar() {
  const menus = [
    { name: 'Test', link: '/Test', icon: BsPen },
    { name: 'Interview', link: '/Interview', icon: GiTalk },
    { name: 'MyPage', link: '/mypage', icon: AiOutlineUser }, // 추가
  ];
  ...
  return (
    ...
    duration-500 text-grey-100`}
        ...
        <div className='flex justify-end space-x-3 font-semibold p-5 px-3 '>
          <Link to='/signup'>
            <div className='font-semibold text-center px-3'>회원가입</div>
          </Link>
          <Link to='/login'>
            <div className='font-semibold text-center'>로그인</div>
          </Link>
        </div>
      </header>
    </>
